# -*- coding: utf-8 -*-
"""
Created on Sat Sep  6 16:39:55 2025
...
data_path = r'C:\Users\rabie\Dropbox\Cyber Security\LargeScaleData1.xlsx'
coef = np.load(r'C:\Users\rabie\Dropbox\Cyber Security\data.npy')
coef = coef.T 
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.h1(x))
        x = F.relu(self.h2(x))
        x = self.max_action * torch.tanh(self.h3(x))
        return x
...
class Critic(nn.Module):
    ...
    def Q1(self, x, u):
        """Get Q-value from first critic only (for actor updates)"""
        xu = torch.cat([x, u], 1)
        x1 = F.relu(self.h1(xu))
        x1 = F.relu(self.h2(x1))
        x1 = self.h3(x1)
...
probbbbs = [0.00, 0.05, 0.10, 0.15, 0.20]
itrrr = 2000  # Initial exploration steps
n_games = 4   # Number of training runs
poason = 0.1  # Base attack probability

# Main training loop for different attack probabilities
    ...
    for yt in range(1, n_games):
        ...
        while not done:
            ...
            if cnttr >= itrrr:
                ...
                action = (action + np.random.normal(0, exploration_noise, size=46)).clip(-1, 1)
            ...
            if cnttr > 400000:
                np.save(fr"C:\Users\rabie\Desktop\New project\{p}\TD3_{yt}_difmean.npy", difmean)
                np.save(fr"C:\Users\rabie\Desktop\New project\{p}\TD3_{yt}_scmean.npy", scmean)
                np.save(fr"C:\Users\rabie\Desktop\New project\{p}\TD3_{yt}_QOSmean.npy", QOSmean)
                np.save(fr"C:\Users\rabie\Desktop\New project\{p}\TD3_{yt}_attacked_allocations.npy", 
                       attacked_allocations_mean)
                np.save(fr"C:\Users\rabie\Desktop\New project\{p}\TD3_{yt}_flaged_allocations.npy", 
                       flaged_allocations_mean)
                break
